 ...
 */
async function getMonthlyProductionKwh(year: number, month: number): Promise<number> {
...
function getProofHtmlString(monthLabel: string, kwhTotal: number): string {
  return `
<!DOCTYPE html>
<html>
<head>
...
async function sendSrecNotificationEmail(
...
export const submitSrecToGats = onSchedule({
  ...
  try {
    ...
    try {
      ...
      for (const input of timeidInputs) {
        const val = await activePage.evaluate((el: any) => el.value, input);
        const name = await activePage.evaluate((el: any) => el.name, input);
      ...
      await activePage.evaluate((fieldId: string, value: string) => {
        const el = (globalThis as any).document.getElementById(fieldId);
        if (el) {
          el.value = value;
      ...
      await activePage.evaluate((el: any) => el.click(), uploadLink);
      
      const fileInput: any = await activePage.waitForSelector("#ucMeterPictures_TextBox0_Input");
      
      // Upload using path
      await fileInput!.uploadFile(tempFilePath);
      
      // Dispatch change event to let DevExpress client-side code register the file
      await activePage.evaluate((el: any) => {
      ...
      await Promise.all([
        activePage.waitForNavigation({ waitUntil: "networkidle0" }).catch(() => null),
        activePage.evaluate((el: any) => el.click(), backBtn)
      ]);
